...
Download_SSR(){
  cd $cur_path || exit 1
	...
	wget -N --no-check-certificate https://github.com/ZhangQi1996/ssr_tools/raw/master/shadowsocksr-manyuser.tgz
	#git config --global http.sslVerify false
	#env GIT_SSL_NO_VERIFY=true git clone -b manyuser https://github.com/ToyoDAdoubiBackup/shadowsocksr.git
	#[[ ! -e ${ssr_folder} ]] && echo -e "${Error} ShadowsocksR服务端 下载失败 !" && exit 1
	[[ ! -e "shadowsocksr-manyuser.tgz" ]] && echo -e "${Error} ShadowsocksR服务端 压缩包 下载失败 !" && rm -rf shadowsocksr-manyuser.tgz && exit 1
	[[ -e /tmp/shadowsocksr-manyuser/ ]] && rf -rf /tmp/shadowsocksr-manyuser/
	tar -zxf shadowsocksr-manyuser.tgz -C /tmp
	[[ ! -e /tmp/shadowsocksr-manyuser ]] && echo -e "${Error} ShadowsocksR服务端 解压失败 !" && rm -rf shadowsocksr-manyuser.tgz && exit 1
	mv -f /tmp/shadowsocksr-manyuser $ssr_folder
	[[ ! -e $ssr_folder ]] && echo -e "${Error} ShadowsocksR服务端 重命名失败 !" && rm -rf shadowsocksr-manyuser.tgz && rm -rf "/tmp/shadowsocksr-manyuser/" && exit 1
	rm -rf shadowsocksr-manyuser.tgz
